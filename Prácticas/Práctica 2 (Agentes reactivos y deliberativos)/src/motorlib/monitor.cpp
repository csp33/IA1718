...
void MonitorJuego::inicializar(int mix, int miy) {
  ...
  if ((nivel == 1) or (nivel == 2)) {
    ...
    nueva_entidad(new Entidad(jugador, jugador_, norte, mix, miy, new Jugador3D(""), new ComportamientoJugador(getMapa()->getNFils()), objX, objY, 2000));
  }
  ...
  if (nivel > 1) {
    for (int i=0; i<naldeanos; i++) {
      ...
      while(not(((celdaRand == 'T' or celdaRand == 'S') and getMapa()->entidadEnCelda(mix, miy) == '_')));
